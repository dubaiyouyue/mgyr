...
class Page extends Think {
	...
	public function  show($nxxxlllpage){
		...
		$outputnewxlxx = '';
		$outputnewxlxx .= '<li><a class="a1">'.'<p>'.$this->totalRows.L('page_item').'</p>'.'<p>'.$this->totalRows.L('page_item').'</p>'.'</a></li>';
		$outputnewxlxx .= '<li><a href="'.$this->pageurl($urlrule, 1,$this->parameter).'">'.'<p>'.L('first_page').'</p>'.'<p>'.L('first_page').'</p>'.'</a></li>';
		$outputnewxlxx .= '<li><a href="'.$this->pageurl($urlrule, $pre_page,$this->parameter).'">'.'<p>'.L('previous').'</p>'.'<p>'.L('previous').'</p>'.'</a></li>';
		...
		if($this->totalPages <= $show_nums){
			for($i = 1;$i<=$this->totalPages;$i++){
				if($i == $this->nowPage){
					$output .= '<span>'.$i.'</span>';
					$outputnewxlxx .= '<li><a><p class="fy_one">'.$i.'</p><p>'.$i.'</p></a></li>';
				}else{
					$output .= '<a href="'.$this->pageurl($urlrule,$i,$this->parameter).'">'.$i.'</a>';
					$outputnewxlxx .= '<li><a href="'.$this->pageurl($urlrule,$i,$this->parameter).'">'.'<p>'.$i.'</p>'.'<p>'.$i.'</p>'.'</a></li>';
				}
			}
		}else{
		...
		$outputnewxlxx .='<li><a href="'.$this->pageurl($urlrule,$next_page,$this->parameter).'">'.'<p>'.L('next').'</p>'.'<p>'.L('next').'</p>'."</a></li>"; 
		$outputnewxlxx .='<li><a href="'.$this->pageurl($urlrule,$this->totalPages,$this->parameter).'">'.'<p>'.L('Last_page').'</p>'.'<p>'.L('Last_page').'</p>'."</a></li>";
		// if($nxxxlllpage) return $outputnewxlxx; 关闭自定义分页
