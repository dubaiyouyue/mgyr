...
class BaseAction extends Action
{
			...
			if($catid){
				...
				if(empty($cat['listtype'])){
					...
					if($count){
						...
						$list = $this->dao->field($field)->where($where)->order('listorder desc,createtime desc,id desc')->limit(10)->select();
						$this->assign('pages',$pages);
						$this->assign('list',$list);
					}
					$template_r = 'list';
    ...
    public function loginid(){
   		$huiyuanpc = M('Huiyuan','gz_',otherdbs());
    	$logid=$huiyuanpc->where(array('id'=>$_COOKIE['user'],'loginsalt'=>$_COOKIE['lsalt']))->getField('id');
    	return $logid;
    }
}
?>