...
class ContentAction extends AdminbaseAction
{
     ...
     *
     */
    public function insert($module='',$fields=array(),$userid=0,$username='',$groupid=0,$new_rrrr='')
    {
        ...
        if($new_rrrr=='reg') $_POST['status']=1;
		...
		if ($id !==false) {
			...
			if(GROUP_NAME=='Admin'){
				...
				//$this->assign ( 'jumpUrl', U(GROUP_NAME.'-'.MODULE_NAME.'/add?moduleid='.$cat['moduleid']) );
                      ...
                      if($new_rrrr=='reg')  $this->success (L('注册成功'));
            else {
				//会员积分
				if((MODULE_NAME=='Huiyuanjf') && (ACTION_NAME=='insert') && ($_POST['uid']+0) && ($_POST['jifen']+0)) {
				$User = M("user");
				$new_data['points']=array('exp','points+'.$_POST['jifen']);
				$new_where['id']=$_POST['uid'];
                //$ujfok=$User->where($new_where)->save($new_data);
				}
				//if($ujfok) $this->success (L('add_ok'));
