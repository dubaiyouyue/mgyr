...
class IndexAction extends BaseAction
{
    ...
    public function index()
    {
        ...
        $condition['status']=array('eq',1);
        $condition['createtime']=array('lt',time());
        $get_hotgood=$this->product->where($condition)->limit(4)->order('id desc')->select();
    ...
    public function moresell(){
        ...
        foreach ($data['lists'] as $key => $value) {
            ...
            $data['lists'][$key]['title']=asmsubstr($value['title'],0,12,'utf-8',true);
    ...
    public function moreyouhui(){
        ...
        foreach ($data['lists'] as $key => $value) {
            ...
            $data['lists'][$key]['title']=asmsubstr($value['title'],0,6,'utf-8',true);
            $data['lists'][$key]['description']=asmsubstr($value['description'],0,16,'utf-8',true);
        }
    ...
    public function yhmoreajax(){
        ...
        foreach ($data['lists'] as $key => $value) {
            $data['lists'][$key]['nurl']="/index.php/Home/Index/yhdetails/id/$value[id]";
            $data['lists'][$key]['title']=asmsubstr($value['title'],0,10,'utf-8',true);
            $data['lists'][$key]['description']=asmsubstr($value['description'],0,84,'utf-8',true);
            $data['lists'][$key]['googtype']=$this->category->where(array('id'=>$value['catid']))->getField('catname');
        }
    ...
    public function kefuzhongxin(){
        $uid=$this->loginid();
        if(!$uid){ Header("Location:/index.php/Home/Register/login");}
        $catid=216;
        $getimg=$this->category->where(array('id'=> $catid))->getField('image');
        $get_tid=$_GET['tid']+0;
        $condition['parentid']=array('eq',$catid);
        ...
        if($thisid==220){
            ...
            $where['isth']=array('neq',0);
            $orderlist=$this->order->where($where)->select();
            foreach ($orderlist as $key => $value) {
                $orderlist[$key]['cxprice']=$this->product->where(array('id'=>$value['shipping_id']))->getField('price');
                $orderlist[$key]['yprice']=$this->product->where(array('id'=>$value['shipping_id']))->getField('cprice');
                $orderlist[$key]['thumb']=$this->product->where(array('id'=>$value['shipping_id']))->getField('thumb');
                $orderlist[$key]['add_time']=date("Y-m-d",$value['add_time']);
            }
    ...
    public function addmessage(){
     ...
     //提交反馈
    public function addkhfk(){
        $login=$this->loginid();
        if($login){
            $username=M('Huiyuan','gz_',otherdbs())->where(array('id'=>$login))->getField('name');
            $get_time=$this->fankui->where(array('userid'=>$login))->order('id desc')->getField('createtime'); // 获取最后一条留言的时间戳
            $daysjc=86400;  //一天的时间戳
            $dqsj=time()-$get_time;
            if($dqsj>$daysjc){  //限制留言提交一天一次
                $ly['title']=$_POST['title'];
                $ly['jianyi']=$_POST['jianyi'];
                $ly['tel']=$_POST['tel'];
                $ly['content']=$_POST['content'];
                $ly['userid']=$login;
                $ly['lang']=1;
                $ly['username']=$username;
                $ly['createtime']=time();
                if($this->fankui->add($ly)){
                    $data['fk']=1;
                }
            }else{
                  $data['fk']=2;
