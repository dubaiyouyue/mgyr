...
class MemberAction extends BaseAction{
     ...
     function _initialize(){    
        ...
        $this->hyxfjl       = M('hyxfjl','gz_',otherdbs());
    }
    ...
    public function xggrzl(){
        ...
        if($_POST){
            ...
            $get_tel_id=$this->huiyuanpc->where(array('tel'=>$_POST['tel']))->getField('id');
            if($get_tel_id!=$uid){     
                //排除相同手机号码
                $data['xgxx']=4;
                $this->ajaxReturn($data);
            ...
            $get_email_id=$this->huiyuanpc->where(array('email'=>$_POST['email']))->getField('id');
            if($get_email_id!=$uid){  
                 //排除相同邮箱             
                $data['xgxx']=5;
                $this->ajaxReturn($data);
            ...
            if($this->huiyuanpc->where(array('id'=>$uid))->save($datab)){
    ...
    public function score(){
        ...
        $getjifen= M('jifenjilu','gz_',otherdbs())->where(array('uid'=>$login))->order('id desc')->select(); 
    ...
    public function mycart(){
        ...
        foreach ($get_carts as $key => $value) {
            $get_carts[$key]['kcs']=$this->thisku($value['product_id'],$value['cm'],$value['ys']);
      ...
      function del_cart(){
        ...
        if($login){
            ...
            if($get_good['type']!=2&&$get_cart['number']>0){
                ...
                if($this->cart->where($condition)->delete()&&$vis==1){
                 ...
                 if($this->cart->where($condition)->delete()){
                    $date['del']=1;
                }else{
                    $date['del']=2;
                 }
    ...
    //                 $get_goodkc=$this->product->where(array('id'=>$value['product_id']))->getField('kc');
    //                 //  检测库存
    //                 $kus=$this->thisku($value['product_id'],$value['cm'],$value['ys']);
    //                 if($kus>=$value['number']){
    //                     $vis=$this->kubd($value['ys'],$value['cm'],$get_goodkc,$kus,$value['number'],$value['product_id']);
    //                 }else{
    //                     $date['apjs']=6;
    //                     $date['number']=$kus;                           //剩余库存数量
    //                     $date['name']=$value['product_name'];     //商品名称
    //                     $date['ys']=$value['ys'];                       //商品颜色
    //                     $date['cm']=$value['cm'];                       //商品尺寸
    //                     $this->ajaxReturn($date);
    //                     return false;
    //                 }
                    
    //                 $this->cart->where(array('userid'=>$login,'id'=>$value['id']))->setField('type',1);   //已过结算的数据
    ...
    function ajax_js(){
        ...
        if($login){
            ...
            $this->order->where(array('pay_status'=>1,'userid'=>$login))->setField('pay_status',0);//过滤数据
            foreach ($get_mycat as $key => $value) {
                    $getprice=$this->product->where(array('id'=>$value['product_id']))->getField('price');
                    $datadn['amount']=$getprice;
                    $datadn['cm']=$value['cm'];
                    $datadn['ys']=$value['ys'];
                    $datadn['number']=$value['number'];
                    $datadn['userid']=$login;
    ...
    function cgtjdd(){
        ...
        if($get_tj){
            ...
            // $condition['pay_status']=array('eq',1);
            // $condition['userid']=array('eq',$login);
            $appxg['addr']=$get_dz;  //收货地址
            $get_tel=$ddzzwx['tel']?$ddzzwx['tel']:$user['tel'];
            $get_name=$ddzzwx['xm']?$ddzzwx['xm']:$user['name'];
            $appxg['tel']=$get_tel;   //联系电话
            $appxg['shrxm']=$get_name; //收货人姓名
            $appxg['pay_time']=time(); //支付时间
        ...
        foreach ($news_dd_list as $k => $va) {
             ...
             $getjifen=$this->product->where(array('id'=>$va['shipping_id']))->getField('jifen');
             $sum += ($news_dd_list[$k]['gprice'] * $va['number']);
             $timeqx=$va['add_time'];
             $jifenes=$va['number']*$getjifen;
             $sjifen=$va['jifen'];
        }
        if($sjifen){
            $jifenq=$sjifen/$confall['jifendk'];
        }else{
            $jifenq=0;
        }
        $youfei=$confall['youfei'];
        // print_r($confall['youfei']);die;
        // 总金额=商品总金额+邮费-积分抵扣
        $sum=$sum+$youfei-$jifenq;
        $qxsj=$timeqx+60*30;
    ...
    */
    ...
    function ordertermpc(){
        ...
        if($get_order){
            ...
            $this->order->where($condition)->setField('number',0);
            unset($condition);
        }
    }
    // 申请退换
    //提醒数量
    public function tcshenqin(){
            $get_id=$_POST['get_id']+0;
            $get_order=$this->order->where(array('id'=>$get_id))->getField('oid');
            $get_idss=$this->hyxfjl->where(array('oid'=>$get_order))->getField('ids');
            $get_count=$this->order->where(array('id'=>array('in',$get_idss)))->count();
            if($get_count>1){
                $data['shul']=2;
            }else{
                $data['shul']=1;
    ...
    public function sqtk(){
        $uid=$this->loginid();
        $get_id=$_GET['id']+0;  //传参数
        if($_POST){
            if($uid){
                $orderid=$_POST['orderid']+0;
                $condition['orderid']=$get_id;
                $condition['userid']=$uid;
                if($this->tkth->where($condition)->find()){
                    $get_oids=$this->order->where(array('id'=>$orderid))->getField('oid');
                    $neswsq['createtime']=time();
                    $neswsq['lang']=1;
                    $neswsq['tkzt']=0;           //申请状态0申请中；1申请通过；2客户取消；
                    $neswsq['title']=$_POST['sqtype'];            //申请类型
                    $neswsq['bftel']=$_POST['bftel'];             //联系电话
                    $neswsq['yx']=$_POST['yx'];                   //邮箱
                    $neswsq['content']=$_POST['content'];         //退换理由
                    if($this->tkth->where($condition)->save($neswsq)&&$this->order->where(array('oid'=>$get_oids))->setField('isth',1)){
                        $data['sl']=1;
                   ...
                   unset($condition);
                    ...
                    $get_order=$this->order->where(array('id'=>$orderid))->field('id,sn,oid')->find();
                    // $get_idss=$this->hyxfjl->where(array('oid'=>$get_order))->find();
                    // $get_count=$this->order->where(array('id'=>array('in',$get_idss)))->count();
                    $sq['orderxx']=$get_order['sn'];
                    $sq['orderid']=$get_order['id'];
                    $finduser=$this->huiyuanpc->where(array('id'=>$uid))->field('tel, name')->find();
                    $sq['tel']=$finduser['tel'];
                    $sq['title']=$_POST['sqtype'];            //申请类型
                    $sq['bftel']=$_POST['bftel'];             //联系电话
                    $sq['yx']=$_POST['yx'];                   //邮箱
                    $sq['content']=$_POST['content'];         //退换理由
                    $sq['createtime']=time();
                    $sq['userid']=$uid;
                    $sq['lang']=1;
                    $sq['username']=$finduser['name'];
                   if($this->tkth->add($sq)&&$this->order->where(array('oid'=>$get_order['oid']))->setField('isth',1)){
                        $data['sl']=1;
        ...
        $this->assign('orderid',$get_id);
        $this->display();
    }

    // 取消申请
    public function yhqxsq(){
        $get_id=$_POST['getid']+0;                    //订单id
        $uid=$this->loginid();
        if($uid){
            $tkthid=$this->order->where(array('id'=>$get_id))->field('id,oid')->find();            //获取申请数据的id
            $xgzt['tkzt']=2;
            $xgzt['lang']=0;
            if($this->tkth->where(array('orderid'=>$tkthid['id'],'userid'=>$uid))->save($xgzt)&&$this->order->where(array('oid'=>$tkthid['oid'],'userid'=>$uid))->setField('isth',0)){
                $data['qx']=1;
            }else{
                $data['qx']=2;
            }
