		...
		if($appid){
			...
			if($f['token']==$_SESSION['login_app_token']){
				if($f['sinauid']){
					...
					$url='/index.php/Home/Register/bdsian';
					...
					$url='/index.php/Home/Register/bdqq';
				...
				if($uid){
					$lw['id']=$uid;
					$lok=M('Huiyuan','gz_',otherdbs())->where($lw)->limit(1)->save($xgmm);
					$url='/index.php/Home/Registerapp/index.html';
				}
				header('Location:'.$url);
				exit();
			}
