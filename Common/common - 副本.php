...
function content_pages($num, $p,$pageurls) {
	...
	if($p>0) {
		$perpage = $p == 1 ? 1 : $p-1;
		if($perpage==1){
			$multipage .= '<li><a class="a1" href="'.$pageurls[$perpage][0].'">'.'<p>'.L('previous').'</p>'.'<p>'.L('previous').'</p>'.'</a></li>';
		}else{
			$multipage .= '<li><a class="a1" href="'.$pageurls[$perpage][1].'">'.'<p>'.L('previous').'</p>'.'<p>'.L('previous').'</p>'.'</a></li>';
		}
		if($p==1) {
			$multipage .= ' <li><a><p class="fy_one">1</p><p>1</p></a></li>';
		} elseif($p>6 && $more) {
			$multipage .= ' <li><a href="'.$pageurls[1][0].'"><p>1</p><p>1</p></a></li>..';
		} else {
			$multipage .= ' <li><a href="'.$pageurls[1][0].'"><p>1</p><p>1</p></a></li>';
		}
	}
	for($i = $from; $i <= $to; $i++) {
		if($i != $p) {
			$multipage .= ' <li><a href="'.$pageurls[$i][1].'">'.'<p>'.$i.'</p>'.'<p>'.$i.'</p>'.'</a></li>';
		} else {
			$multipage .= ' <li><a><p class="fy_one">'.$i.'</p><p>'.$i.'</p></a></li>';
		}
	}
	if($p<$pages) {
		if($p<$pages-5 && $more) {
			$multipage .= ' ..<li><a href="'.$pageurls[$pages][1].'">'.'<P>'.$pages.'</p>'.'<P>'.$pages.'</p>'.'</a> </li><li><a class="a1" href="'.$pageurls[$p+1][1].'">'.'<p>'.L('next').'</p>'.'<p>'.L('next').'</p>'.'</a></li>';
		} else {
			$multipage .= ' <li><a href="'.$pageurls[$pages][1].'">'.'<p>'.$pages.'</p>'.'<p>'.$pages.'</p>'.'</a> </li><li><a class="a1" href="'.$pageurls[$p+1][1].'">'.'<p>'.L('next').'</p>'.'<p>'.L('next').'</p>'.'</a></li>';
		}
	} elseif($p==$pages) {
		$multipage .= ' <li><a><p class="fy_one">'.$pages.'</p><p>'.$pages.'</p></a></li> <li><a class="a1" href="'.$pageurls[$p][1].'">'.'<p>'.L('next').'</p>'.'<p>'.L('next').'</p>'.'</a></li>';
	}
	return $multipage;
}
